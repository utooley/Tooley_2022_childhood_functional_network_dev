...
output_image_directory="~/Dropbox/projects/in_progress/within_between_network_conn_CBPD/output/figures/brains/revisions/"
output_directory="~/Dropbox/projects/in_progress/within_between_network_conn_CBPD/data/ms_data/revisions"
...
#Nodal age effects
n92_all_edges<- readRDS("~/Documents/projects/in_progress/within_between_network_conn_CBPD/data/imageData/gsr_spkreg_fd0.25dvars1.75_drpvls/n83_all_edges.RData");n92_all_edges <- n92_all_edges[,-1]
...
parcel_sd_pvals<- lapply(names(n92_nodal_strength_data[,1649:2048]), function(x) { summary(lm(as.formula(paste0(x,"~age_scan+male+fd_mean_avg+avgweight+totalSizet")), data=n92_nodal_strength_data))$coef[2,4]})
parcel_sd_pvals <- unlist(parcel_sd_pvals)
parcel_sd_pvals_fdr <- cbind(parcel_sd_pvals,p.adjust(parcel_sd_pvals,method = "fdr"))
#get age betas
parcel_sd_betas<- lapply(names(n92_nodal_strength_data[,1649:2048]), function(x) { lm.beta(lm(as.formula(paste0(x,"~age_scan+male+fd_mean_avg+avgweight+totalSizet")), data=n92_nodal_strength_data))$standardized.coefficients[[2]]})
...
#for (pvalue in pvalues){
  ...
  colFn_diverging = colorRampPalette(c("white","#E0011C"));makecmap_options=list('colFn'=colFn_diverging, range=c(0,10)) 
  rglactions=list("snapshot_png"=paste0(output_image_directory,"mr_pos_pvals_", pvalue,"regions.png"), 'shift_hemis_apart'= T) #purple="#7502E3", red=#E0011C, blue=#3602D9
...
# Supplementary Pipeline: Replication using 0.25mm censoring threshold, n=83 sample --------

output_image_directory="~/Dropbox/projects/in_progress/within_between_network_conn_CBPD/output/figures/brains/revisions/n83_sample_censoring0.25mm_pipeline_exclude_0.5mm/"
output_directory="~/Dropbox/projects/in_progress/within_between_network_conn_CBPD/data/ms_data/revisions/n83_pipeline/"
...
#Use main_unique instead
lm_within_sys_age <- lm(mean_within_sys~age_scan+male+fd_mean_avg+avgweight+totalSizet, data=main_unique)
summary_within_sys_age <- apa_print(Anova(lm_within_sys_age));lm_eta_within_sys_age <- eta_squared(Anova(lm_within_sys_age), ci=0.95)
outcomes <- main_unique %>% ungroup() %>% dplyr::select(mean_within_sys,mean_between_sys,system_segreg, modul_avg, part_coef,avgclustco_both) %>% names()
for (outcome in outcomes){
  model<-paste0("lm_",outcome,"_age")
  formula<-formula(paste0(outcome, '~age_scan+male+fd_mean_avg+avgweight+totalSizet'))
...
visreg(lm_mean_between_sys_age,"age_scan", main="Mean between-system connectivity", ylab="", xlab="Age at scan (years)")
visreg(lm_mean_within_sys_age,"age_scan", main= "Mean within-system connectivity", ylab="", xlab="Age at scan (years)")
visreg(lm_system_segreg_age,"age_scan", main="System segregation", ylab="", xlab="Age at scan (years)")
visreg(lm_modul_avg_age,"age_scan", main="Modularity quality index (Q)", ylab="", xlab="Age at scan (years)")
visreg(lm_part_coef_age,"age_scan", main="Average participation coefficient", ylab="", xlab="Age at scan (years)")
visreg(lm_avgclustco_both_age,"age_scan", main="Average clustering coefficient", ylab="", xlab="Age at scan (years)")

networks_age_pvals_fdr #FDR-corrected
outcomes <- main_unique %>% ungroup() %>% dplyr::select(mean_within_sys,sys1to3,sys3to4,sys3to7,sys4to7) %>% names()
for (outcome in outcomes){
  model<-paste0("lm_",outcome)
  formula<-formula(paste0(outcome, '~age_scan+male+fd_mean_avg+avgweight+totalSizet'))
  assign(model, lm(formula, data=main_unique))
...
# goback
# 
saveRDS(vect,"~/Documents/projects/in_progress/within_between_network_conn_CBPD/data/imageData/gsr_spkreg_fd0.25dvars1.75_drpvls/edgewise_age_effects_all_cov_n83.Rdata")
...
edgewise_age_betas <- unlist(edgewise_age_betas)
#save for reloading in future
save(edgewise_age_pvals_fdr, edgewise_age_betas,file= "~/Documents/projects/in_progress/within_between_network_conn_CBPD/data/imageData/gsr_spkreg_fd0.25dvars1.75_drpvls/edgewise_age_effects_all_cov_n85.Rdata")

#load back in edge age effects
load("~/Documents/projects/in_progress/within_between_network_conn_CBPD/data/imageData/gsr_spkreg_fd0.25dvars1.75_drpvls/edgewise_age_effects_all_cov_n83.Rdata")
...
schaefer_atlas_region_names_lh = get.atlas.region.names('Schaefer2018_400Parcels_7Networks_order', template_subjects_dir = subjects_dir,template_subject=subject_id, hemi="lh");
schaefer_atlas_region_names_rh = get.atlas.region.names('Schaefer2018_400Parcels_7Networks_order', template_subjects_dir = subjects_dir,template_subject=subject_id, hemi="rh");
output_image_directory="/Users/utooley/Dropbox/projects/in_progress/within_between_network_conn_CBPD/output/figures/replicate_0.25mm_censor_pipeline_n83/"
pvalues=c(0.01,0.001,0.0001)
pvalue=0.001
#Use different pval thresholds and plot parcels that have age-nonlin-sig edges at that threshold
pos_edges <- edge_age_pvals_mat;pos_edges[edge_age_betas_mat<0] <- NA
...
#for (pvalue in pvalues){
  ...
  #colormap
  colFn_diverging = colorRampPalette(c("white","#3602D9"));makecmap_options=list('colFn'=colFn_diverging, range=c(0,10)) 
  rglactions=list("snapshot_png"=paste0(output_image_directory,"age_neg_pvals_", pvalue,"regions.png")) #purple="#7502E3", red=#E0011C, blue=#3602D9
  coloredmeshes=vis.region.values.on.subject(subjects_dir, 'fsaverage6', 'Schaefer2018_400Parcels_7Networks_order',  num_of_sig_age_edges_lh,
  num_of_sig_age_edges_rh, makecmap_options = makecmap_options, "inflated", views="t4", draw_colorbar = T, rgloptions = rgloptions, rglactions = rglactions)
  coloredmesh.plot.colorbar.separate(coloredmeshes, png_options = list(filename= paste0(output_image_directory, "age_neg_pvals_", pvalue,"colorbar.png")))
#}
#get edge matrix, make average nodal strength
n92_all_edges<- readRDS("~/Documents/projects/in_progress/within_between_network_conn_CBPD/data/imageData/gsr_spkreg_fd0.25dvars1.75_drpvls/n83_all_edges.RData");n92_all_edges <- n92_all_edges[,-1]
...
nodes <- which(parcel_sd_pvals_fdr[,2] <0.05);nodes <- names(n92_nodal_strength_data[,1645+nodes]) #none
...
visreg(lm_reasoning_sys1to3, "sys1to3", main= "Matrix Reasoning (scaled score)", ylab="Scaled score", xlab="VIS to DA connectivity")
visreg(lm_reasoning_sys3to7, "sys3to7", main= "Matrix Reasoning (scaled score)", ylab="Scaled score", xlab="DM to DA connectivity")
visreg(lm_reasoning_sys4to7)

## Plot matrix reasoning on brain
#load back in edge age effects
load("~/Documents/projects/in_progress/within_between_network_conn_CBPD/data/imageData/gsr_spkreg_fd0.25dvars1.75_drpvls/edgewise_mr_effects_all_cov_n83.Rdata")
...
#for (pvalue in pvalues){
  ...
  #colormap
  colFn_diverging = colorRampPalette(c("white","#E0011C"));makecmap_options=list('colFn'=colFn_diverging, range=c(0,10)) 
  rglactions=list("snapshot_png"=paste0(output_image_directory,"mr_pos_pvals_", pvalue,"regions.png"), 'shift_hemis_apart'=TRUE) #purple="#7502E3", red=#E0011C, blue=#3602D9
  coloredmeshes=vis.region.values.on.subject(subjects_dir, 'fsaverage6', 'Schaefer2018_400Parcels_7Networks_order',  num_of_sig_age_edges_lh, 
                                             num_of_sig_age_edges_rh, makecmap_options = makecmap_options, "inflated", views="t9", draw_colorbar = T, rgloptions = rgloptions, rglactions = rglactions)
  coloredmesh.plot.colorbar.separate(coloredmeshes, png_options = list(filename= paste0(output_image_directory, "mr_pos_pvals_", pvalue,"colorbar.png")))
#}

save(list=c("main_unique", "networks_age_pvals_fdr", ls(pattern = "summary_lm_*"), 
            ls(pattern = "eta_lm_*"), ls(pattern = "cohensf_lm_*"), ls(pattern = "beta_lm_*")),
     file = paste0(output_directory,"/data.Rdata"))
